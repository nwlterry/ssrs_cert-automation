...
# 2. Extract ONLY the Common Name (CN) for binding
$DnsNames = @()
if ($NewCertificate.Subject -match 'CN=([^,]+)') { 
    $DnsNames += $Matches[1].Trim()
} else {
    throw "Could not extract a Common Name (CN) from the certificate subject: $($NewCertificate.Subject)"
}

if ($DebugMode) { Write-Output "Extracted Common Name for Binding: $($DnsNames[0])" }
...
foreach ($Config in $SsrsConfigs) {
    ...
    foreach ($App in @('ReportServerWebService', 'ReportServerWebApp')) { 
        ...
        # Reserve URLs (Now strictly using the single Common Name)
